...
public sealed class RuntimeMutationCoordinator
    {
        ...
        public void RegisterPropertyChange(AvaloniaObject target, AvaloniaProperty property, object? oldValue, object? newValue)
        {
        ...
        public bool TryApplyElementRemoval(TreeNode node)
        {
            Dispatcher.UIThread.VerifyAccess();
        ...
        private sealed class PropertyMutation : IRuntimeMutation
        {
            ...
            public PropertyMutation(AvaloniaObject target, AvaloniaProperty property, object? oldValue, object? newValue)
            {
        ...
        private sealed class ElementRemovalMutation : IRuntimeMutation
        {
            ...
            private ElementRemovalMutation(AvaloniaObject parent, AvaloniaObject element, RemovalKind kind, int index, object? storedItem)
            {
            ...
            public static bool TryCreate(TreeNode node, out ElementRemovalMutation mutation)
            {
                mutation = default!;
                ...
                switch (parent)
                {
                    ...
                    case Decorator decorator when decorator.Child == element:
                        mutation = new ElementRemovalMutation(decorator, element, RemovalKind.Decorator, 0, decorator.Child);
                        return true;

                    case ContentControl contentControl when ReferenceEquals(contentControl.Content, element):
                        mutation = new ElementRemovalMutation(contentControl, element, RemovalKind.ContentControl, 0, contentControl.Content);
                        return true;
...
#pragma warning restore CS0618
                        if (containerIndex < 0 && itemsControl.Items is IList list)
                        {
                            ...
                            if (containerIndex >= 0)
                            {
                                ...
                                mutation = new ElementRemovalMutation(itemsControl, container, RemovalKind.ItemsControl, containerIndex, stored);
                                return true;
                            }
                        }
            ...
            public void ApplyRemoval()
            {
                ...
                switch (_kind)
                {
                    ...
                    case RemovalKind.ItemsControl when parent is ItemsControl itemsControl && itemsControl.Items is IList list && _index >= 0 && _index < list.Count:
            ...
            public void ApplyUndo()
            {
                ...
                switch (_kind)
                {
                    ...
                    case RemovalKind.ItemsControl when parent is ItemsControl itemsControl && itemsControl.Items is IList list:
